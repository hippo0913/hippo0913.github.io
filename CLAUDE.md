...
文章写完或修改完成后，自动提交并推送到 GitHub，无需等待用户确认：
...
- 新文章：`post: 文章标题`
- 修改文章：`post: 修改描述`
- 配置/重构：`docs:` / `refactor:` / `feat:` 等

推送后约 1 分钟自动上线 https://hippo0913.github.io
...
3. **一次一件事**：不要同时创建多篇文章，完成一篇确认后再写下一篇
4. **自动提交推送**：文章或配置修改完成后，自动 commit + push，无需等待确认
